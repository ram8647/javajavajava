...
\label{fig-keybrdrdrclass}
\label{pg-fig-keybrdrdrclass}}
\end{figure}
...
\label{fig-greeterapp}
\label{pg-fig-greeterapp}}
\end{figure}
...
\JavaRule{Top-level Windows.}{Top-level Java windows cannot contain
other top-level windows as components. }
...
\end{lstlisting}
\end{jjjlisting}
\jjjprogstop{Definition of the {\tt GreeterGUIPanel} class.
\label{fig-guipanel}
\label{pg-guipanel}}
\end{figure}
...
Given these changes, we would then change the applet's {\tt init()} method
to the following:
...
\begin{figure}[tb]
%%%\figaleft{chptr04/greeterapplet.eps}  
\figaleftscaled{chptr04/greeterapplet.eps}{0.75}  
